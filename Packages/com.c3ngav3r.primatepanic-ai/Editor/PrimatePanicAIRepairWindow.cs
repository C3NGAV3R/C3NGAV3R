...
{
    public sealed class PrimatePanicAIRepairWindow : EditorWindow
    {
        private string request = "";
        private string status = "READY — safe scene repair online.";
        private Vector2 scroll;

        [MenuItem("Tools/Primate Panic AI - FIXED / Repair Agent")]
        public static void Open() => GetWindow<PrimatePanicAIRepairWindow>("Primate Panic AI FIXED");

        private void OnGUI()
        {
            EditorGUILayout.LabelField("Primate Panic AI — FIXED REPAIR AGENT", EditorStyles.boldLabel);
            EditorGUILayout.HelpBox("This repair agent scans the real active scene, resolves real hierarchy objects, refuses guessed targets, and verifies changes. It never creates replacement UI for an existing button.", MessageType.Info);
        ...
        private void RunRepair()
        {
            ...
            status = "No safe repair matched. Scan first; this tool will not guess an unrelated object.";
        }
        ...
        private void RepairBoth()
        {
            ...
            ItemToggleController c = GetOrCreateController();
            ...
            status = "REPAIR COMPLETE\n\n" +
                ...
                "Existing buttons were wired to persistent OnClick listeners. No replacement Canvas/Button was created.";
        }
        ...
        private void RepairOne(string label, string[] keys, bool hammer)
        {
            ...
            {
                status = "REPAIR BLOCKED — I will not guess.\n\n" + Missing(hammer ? item : null, hammer ? null : item, hammer ? button : null, hammer ? null : button);
                return;
            }
            ItemToggleController c = GetOrCreateController();
            SetField(c, hammer ? "kickHammer" : "jetpack", item);
            ReplacePersistent(button, hammer ? (UnityEngine.Events.UnityAction)c.ToggleKickHammer : c.ToggleJetpack);
            Save();
            status = "REPAIRED " + label + "\nObject: " + PathOf(item) + "\nButton: " + PathOf(button.gameObject) + "\nScene saved.";
        }

        private ItemToggleController GetOrCreateController()
        {
            ItemToggleController existing = Resources.FindObjectsOfTypeAll<ItemToggleController>()
                .FirstOrDefault(x => x != null && x.gameObject.scene == SceneManager.GetActiveScene());
            if (existing != null) return existing;
            GameObject go = new GameObject("PrimatePanic_ItemToggleController");
            Undo.RegisterCreatedObjectUndo(go, "Create item toggle controller");
            return go.AddComponent<ItemToggleController>();
        }

        private static void SetField(ItemToggleController c, string name, GameObject value)
        {
            FieldInfo f = typeof(ItemToggleController).GetField(name, BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        private static int Score(GameObject go, string[] keys, bool button)
        {
            ...
            int s = button ? 25 : 0;
            foreach (string k0 in keys)
            {
                string k = k0.ToLowerInvariant();
                if (n == k) s += 100;
    ...
    public sealed class ItemToggleController : MonoBehaviour
    {
